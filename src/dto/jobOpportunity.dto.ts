...
import { ArrayMaxSize, IsArray, IsEmpty, IsIn, IsMongoId, IsNotEmpty, IsOptional, IsString, Validate, ValidatorConstraint, ValidatorConstraintInterface } from "class-validator";
...
export class JobOpportunityDto {
    ...
    @IsArray()
    @ArrayMaxSize(2)
    @Validate(IsFisrtLessThanSecond, {message: "The first element must be less than the second element"})
    experience: number[];
