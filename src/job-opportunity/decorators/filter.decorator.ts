...
export const FilteringParams = createParamDecorator((data, exec: ExecutionContext): Filtering[] => {
    ...
    filters.forEach((filter) => {
        if (!filter.match(/^[a-zA-Z0-9_]+:(eq|range):[a-zA-Z0-9_,]+$/) && !filter.match(/^[a-zA-Z0-9_]+:(isnull|isnotnull)$/)) {
