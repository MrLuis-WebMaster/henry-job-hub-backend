...
import { JobOpportunityDto, UpdateJobOpportunityDto } from '../dto/jobOpportunity.dto';
...
export class JobOpportunityService {
  ...
  constructor(
    @InjectModel(JobOpportunity.name) private jobOpportunityModule: Model<JobOpportunity>, 
    @InjectModel(User.name)  private userModel: Model<User>
  ){}


  async create(createJobOpportunityDto: JobOpportunityDto): Promise<JobOpportunity>{
    try {
      ...
      if(!userFind){
        throw new UnauthorizedException(`The use with id ${user} doesn´t exist`)
      }
  
      const newJobOpportunity = await this.jobOpportunityModule.create(createJobOpportunityDto)
  ...
  async filterAndFind(sort?: Sorting, filters?: Filtering[]): Promise<JobOpportunity[]>{
    try {
      const order: {[key: string]: any} = getSort(sort)
      const where = getFilters(filters)
      console.log(where)
      const JobOpportunities = await this.jobOpportunityModule.find(where).sort(order).exec()
  ...
  async findCreatedByUser(idUser: string): Promise<JobOpportunity[]>{
    try {
      const jobOpportunities = await this.jobOpportunityModule.find({user: idUser})
      return jobOpportunities
    } catch (error) {
      throw new UnauthorizedException(error) 
    }
  ...
  async update(id: string, updateJobOpportunityDto: UpdateJobOpportunityDto): Promise<JobOpportunity> {
    try {
      const jobOpportunity = await this.jobOpportunityModule.findByIdAndUpdate(id, updateJobOpportunityDto,{
        new:true
      })
