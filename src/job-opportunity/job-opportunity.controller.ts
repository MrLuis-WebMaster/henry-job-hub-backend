...
import { JobOpportunityDto, UpdateJobOpportunityDto } from 'src/dto/jobOpportunity.dto';
...
export class JobOpportunityController {
  ...
  @Post()
  create(@Headers('authorization') token: string, @Body() createJobOpportunityDto: JobOpportunityDto) {
  ...
  findAll(
    ...
    @FilteringParams(['country', 'company', 'user', 'yearsOfexperience', 'mode', 'createdAt']) filter?: Filtering[]
  ) {
  ...
  @Put(':id')
  update(@Param('id') id: string, @Body() updateJobOpportunityDto: UpdateJobOpportunityDto) {
